...
var (
	...
	// Define the files to create/use
	cmdHistory    *os.File
	modHistory    *os.File
	backupHistory *os.File
	errorHistory  *os.File
	serverHistory *os.File
)
...
func InitLogger() {
	...
	serverHistory, err = os.OpenFile(path.Join(cwd, "resources", "logs", "serverhistory.log"), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0666)
	...
	ServerLogger = log.New(serverHistory, "", log.Ldate|log.Ltime)
}
...
//ServerLog logs is the default to handle print to screen and output to file
func ServerLog(data ...interface{}) {
	Logger.Println(data...)
	ServerLogger.Println(data...)
}
